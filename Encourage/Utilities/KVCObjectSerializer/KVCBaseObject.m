...
+ (id)objectForDictionary:(NSDictionary *) inputDict {
    ...
    for (NSString * key in [inputDict allKeys]){
        [self kvcObject:inputDict :key :kvcObject :propertyDict :propertyKeys];
    }
    return [kvcObject autorelease];
}

+(id)kvcObject:(NSDictionary *)inputDict :(NSString *)key :(id)kvcObject :(NSDictionary *)propertyDict :(NSArray *)propertyKeys{
    {
        id propertyValue = [inputDict objectForKey:key];
        
        if ([propertyValue isKindOfClass:[NSDictionary class]]) {
            for (NSString *aKey in [propertyValue allKeys]) {
                [ self kvcObject:propertyValue :aKey :kvcObject :propertyDict :propertyKeys];
        ...
        if ([propertyValue isKindOfClass:[NSArray class]]) {
            for (NSDictionary *dict in propertyValue) {
                for (NSString *theKey in [dict allKeys]) {
                    [self kvcObject:dict :theKey :kvcObject :propertyDict :propertyKeys ];
                }
            }
        }
