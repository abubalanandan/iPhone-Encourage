...
- (void)moveContentview:(UIView *)contentView withDirection:(KGModalDirection)direction andAnimated:(BOOL)animated {
    ...
    CGFloat offset = (direction == KGDirectionUp) ? -100 : 100;
    CGFloat padding = 17;
    CGRect containerViewRect = CGRectInset(contentView.bounds, -padding, -padding);
    containerViewRect.origin.x = containerViewRect.origin.y = 0;
    containerViewRect.origin.x = round(CGRectGetMidX(self.window.bounds)-CGRectGetMidX(containerViewRect));
    containerViewRect.origin.y = round(CGRectGetMidY(self.window.bounds)-CGRectGetMidY(containerViewRect) + offset);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(animated){
            ...
            containerView.alpha = 0;
            containerView.layer.shouldRasterize = YES;
            containerView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, 0.4, 0.4);
            [UIView animateWithDuration:kTransformPart1AnimationDuration animations:^{
                containerView.alpha = 1;
                containerView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, 1.1, 1.1);
            } completion:^(BOOL finished) {
                [UIView animateWithDuration:kTransformPart2AnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
                    containerView.alpha = 1;
                    containerView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, 1, 1);
                } completion:^(BOOL finished2) {
                    containerView.layer.shouldRasterize = NO;
                    [[NSNotificationCenter defaultCenter] postNotificationName:KGModalDidShowNotification object:self];
                }];
...
- (void)closeAction:(id)sender{
    ...
    if ([self.contentViewController respondsToSelector:@selector(removeAllPickers)])
        [self.contentViewController performSelector:@selector(removeAllPickers)];
    
    if ([self.contentViewController respondsToSelector:@selector(didCloseKGModal)]) {
        [self.contentViewController performSelector:@selector(didCloseKGModal)];
    }
...
- (void)removeAllPickers {
    ...
    // Will be overrided on content view controllers.
}

- (void)didCloseKGModal {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [UIView animateWithDuration:kTransformPart2AnimationDuration animations:^{
            ...
            [UIView animateWithDuration:kTransformPart1AnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"kgmodal" object:nil];
            }];
        }];
    });
}
...
- (void)setUserInteractionEnabled:(BOOL)val
{
    self.viewController.view.userInteractionEnabled = val;
    self.closeButton.userInteractionEnabled = val;
